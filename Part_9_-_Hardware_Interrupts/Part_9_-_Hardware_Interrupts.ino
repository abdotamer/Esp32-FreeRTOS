...
// Settings
static const char CMD[] = "avg";              // CMD
static const uint16_t timerDivider = 8;          // Divide 80 MHz by this
static const uint64_t timerMaxCount = 1000000;  // Timer counts to this value
static const uint32_t Delay = 20;             // ms delay
...
enum { CMD_BUF_LEN = 255};  // Number of characters in CMD buffer

// Pins
static const int SensorPin = 35;
...
void IRAM_ATTR onTimer() {
  ...
  if ((idx < BUF_LEN) && (buf_overrun == 0)) {
    write_to[idx] = analogRead(SensorPin);
    idx++;
  }
  ...
  while (1) {
    ...
    if (Serial.available() > 0) {
      ...
      if ((c == '\n') || (c == '\r')) {
        ...
        // Print average value if CMD given is "avg"
        cmd_buf[idx - 1] = '\0';
        if (strcmp(cmd_buf, CMD) == 0) {
...
void setup() {
  ...
  // Start task to handle CMD line interface events. Let's set it at a
  ...
  timerAlarmWrite(timer, timerMaxCount, true);
